...
(const G4int copyNo, G4VPhysicalVolume* physVol) const
{
  ...
  G4int nstb,row,col;
  //Start from south side, skipping over gap, calculate center of cell with copy number. Assgin nstb, row, col
  //shift= how many cells from origion to start row
  //leftNo = number of innermost cell in that row; 
  //rowNo = row of cell (Maxrow-rowNo gives our usual coordiantes
  //Large cells with wrapper
