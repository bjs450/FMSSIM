...
//  fu = new G4FileUtilities();
  ...
  geo1.SetGains("/home/bsumma/simulations/analysis/root12fms/fmstxt", "", "FmsCorrAll1.txt");
  geo2.SetGains("/home/bsumma/TriggerTesting","","FmsCorrVar10Percent.txt");
  geo3.SetGains("/home/bsumma/TriggerTesting","","FmsCorrVar20Percent.txt");
  geo4.SetGains("/home/bsumma/TriggerTesting","","FmsCorrVar30Percent.txt");
...
void PostSim::Digitise(Int_t nn, int k)
{
  ...
  if(nn == 3)
  {
    ...
    if(dummyFirstInEvent==0)
    {
      ...
      QT->TriggersTest();
      for(Int_t qtdum=0; qtdum<9; qtdum++)
      {
	Trig1[qtdum] = (QT->TriggerPass)[qtdum];
	std::cout<<"TRIGGER "<<qtdum<<" = "<<((QT->TriggerPass)[qtdum])<<std::endl;
      }

      QT->decodeQT(nqtGeo2,QtGeo2,0);
      QT->TriggersTest();
      for(Int_t qtdum=0; qtdum<9; qtdum++) Trig2[qtdum] = (QT->TriggerPass)[qtdum];
      QT->decodeQT(nqtGeo3,QtGeo3,0);
      QT->TriggersTest();
      for(Int_t qtdum=0; qtdum<9; qtdum++) Trig3[qtdum] = (QT->TriggerPass)[qtdum];
      QT->decodeQT(nqtGeo4,QtGeo4,0);
      QT->TriggersTest();
      for(Int_t qtdum=0; qtdum<9; qtdum++) Trig4[qtdum] = (QT->TriggerPass)[qtdum];
