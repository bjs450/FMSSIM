...
Airgapwidth=0;
  ...
  airgapCellL = new G4Box("solidAirCellL",
    ...
    CellWidthL/2-Alwidth,
    CellLengthL/2-Alwidth+HalfPhotoCLength+CookieLength/2);
  ...
  airgapCellS = new G4Box("solidAirCellS",
    ...
    CellWidthS/2-Alwidth,
    CellLengthS/2-Alwidth+HalfPhotoCLength+CookieLength/2);
  ...
  /*
  OpSurfaceS -> SetFinish(groundbackpainted);
  OpSurfaceL -> SetFinish(groundbackpainted);
  */
  ...
  /*
  OpSurfaceL -> SetSigmaAlpha(0.1);
  OpSurfaceS -> SetSigmaAlpha(0.1);
  */
  ...
  //BS DELETE LATER
  G4double testarr[nEntries]={0};
//  for(int j=0;j<nEntries;j++)testarr[j]=100;
